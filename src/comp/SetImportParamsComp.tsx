...
export function SetImportParamsComp({
	...
	const [$dictionaries, set$dictionaries] =
		React.useState<TLoadable<Dictionary[]>>(null)
		...
		;(async () => {
			...
			if (_dictionaryId) {
				...
				const dictionaryWithSameName = dictionaries.find(
					(dictionary) =>
						dictionary.name === _importableDictionary.dictionary.name,
				)
						...
						<select
							...
							onChange={(e) => {
								...
								const dictionary = isLoaded($dictionaries)
									? $dictionaries.find((dictionary) => dictionary.id === id)
									: undefined
								_setImportParams((importParams) => ({
									...importParams!,
									dictionary: dictionary || _importableDictionary.dictionary,
									swapLanguages: false,
								}))
							}}
						>
